...
import { Users, Search, ChevronLeft, Calendar, ChevronRight, X, XCircle, CheckCircle } from 'lucide-react';
                        ...
                        {filteredCoaches.map((coach) => (
                            <div key={coach.id}
                                ...
                                className={`group relative flex flex-col items-center p-6 rounded-2xl border transition-all duration-300 hover:scale-[1.02] cursor-pointer
                                    ${coach.status === 'present' ? 'bg-[#152b24]/60 border-emerald-500/30' :
                                        coach.status === 'absent' ? 'bg-[#2b1515]/60 border-rose-500/30' :
                            ...
                            >
                                ...
                                <div className="relative mb-4">
                                    <div className="w-20 h-20 rounded-2xl bg-white/5 flex items-center justify-center overflow-hidden border-2 border-white/10">
                                        {coach.avatar_url ? (
                                            ...
                                            <span className="font-black text-white/50 text-3xl">{coach.full_name[0]}</span>
                                        )}
                                    </div>
                                    {/* Status Indicator */}
                                ...
                                {/* Role */}
                                <p className="text-[10px] text-white/40 font-bold uppercase tracking-wider mb-4">
                                    {coach.role || 'Staff'}
                                </p>
                                ...
                                <div className="w-full space-y-2">
                                    <div className="flex items-center justify-between text-xs">
                                        <span className="text-white/40 font-bold uppercase tracking-wider text-[10px]">Total Time</span>
                                    ...
                                    <div className="flex items-center justify-between text-xs">
                                        <span className="text-white/40 font-bold uppercase tracking-wider text-[10px]">Check In</span>
                                        <span className="text-white font-black">
                                            {coach.checkInTime
                                    ...
                                    <div className="flex items-center justify-between text-xs">
                                        <span className="text-white/40 font-bold uppercase tracking-wider text-[10px]">Check Out</span>
                                        <span className="text-white font-black">
                                            {coach.checkOutTime
                                ...
                                {(() => {
                                    ...
                                    if (isCleaner) {
                                        // Cleaners get both buttons
                                        return (
                                            <div className="w-full mt-4 pt-4 border-t border-white/10 flex gap-2" onClick={(e) => e.stopPropagation()}>
                                                {coach.status !== 'present' && (
                                                    <button
                                                        onClick={() => handleMarkPresent(coach.id)}
                                                        className="flex-1 px-3 py-2 rounded-lg bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 transition-all flex items-center justify-center"
                                                    >
                                                ...
                                                {coach.status !== 'absent' && (
                                                    <button
                                                        ...
                                                        className="flex-1 px-3 py-2 rounded-lg bg-rose-500/20 hover:bg-rose-500/30 text-rose-500 transition-all flex items-center justify-center"
                                                    >
                                        ...
                                        // Other staff (coaches, reception, head coach) only get absent button
                                        return coach.status !== 'absent' ? (
                                            <div className="w-full mt-4 pt-4 border-t border-white/10" onClick={(e) => e.stopPropagation()}>
                                                <button
                                                    onClick={() => handleMarkAbsent(coach.id)}
                                                    className="w-full px-3 py-2 rounded-lg bg-rose-500/20 hover:bg-rose-500/30 text-rose-500 transition-all flex items-center justify-center"
                                                >
                                                    <XCircle className="w-4 h-4" />
                                                </button>
                                            </div>
